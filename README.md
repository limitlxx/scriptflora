...
# Uses Runway Gen-4 Image (gen4_image_turbo). When absent, placeholder images are returned.
RUNWAY_API_KEY=   # also used for Phase 3 video generation
...
- `POST /api/generate-reference-image` — submits N Runway Gen-4 Image (`gen4_image_turbo`) tasks in parallel; accepts `prompt`, `type` (`character` | `style`), `count` (1–4), and an optional `sourceImage` base64 data URL; returns `{ taskIds, simulated }` immediately. The client then polls `GET /api/generate-reference-image?taskIds=…` (every 2.5 s, 2-min deadline) until `{ done: true, images }` is returned. Falls back to placeholder images (simulated mode) when `RUNWAY_API_KEY` is absent; requires an active LWC session
